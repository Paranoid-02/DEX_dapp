...
contract('Exchange',([deployer,feeAccount,user1,user2]) => {
    ...
    describe('make orders',async()=>{
        let result
        beforeEach(async()=>{
            result = await exchange.makeOrders(token.address,tokens(1),ETHER_ADDRESS,ether(1),{from:user1})
        })
    ...
    describe('order actions', async() => {
        ...
        beforeEach(async () => {
          ...
          await exchange.makeOrders(token.address, tokens(1), ETHER_ADDRESS, ether(1), { from: user1 })
        })
